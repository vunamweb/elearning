...
/* vu nam */
  ...
  if(isset($_POST['register']))
 {
    ...
    if($class=="")
    {
		...
		$headers .= 'From: chung shi Fernlehrgang <info@chung-shi.com>' . "\r\n";
 ...
 if(isset($_POST['test']))
 {
     ...
     if($_POST['status']==0)
     {
        ...
        else
        {
            ...
            //if faild <2 times
              ...
              echo 'Sie haben die Fernlehrgang-Pr&uuml;fung zum '.$count.'. Mal nicht bestanden.<br>Sie k&ouml;nnen die Fernlehrgang-Pr&uuml;fung noch '.$rest_test.' Mal durchf&uuml;hren.';
     ...
     else
     {
        ...
        if($test<6)
        {
          ...
          safe_query($sql);
          echo 'Herzlichen Gl&uuml;ckwunsch. Sie haben die Fernlehrgangs-Pr&uuml;fung bestanden.<br>In 2 Wochen erhalten Sie die Freigabe F&uuml;r die n&auml;chste Pr&uuml;fung.';
        }
        elseif($test == 6)
        {
          $sql="insert into user_test_active(id_user,test,active)values(".$id_user.",".$test.",1)";
          safe_query($sql);
          echo 'Herzlichen Gl&uuml;ckwunsch. Sie haben den Fernlehrgang bestanden.<br>Sie erhalten von uns Ihr Zertifikat per E-Mail.';
          // create pdf
          // PDF ready
          $html='<div><img src="img/logo.png"/></div>
                 <br>
                 <div>Congratulation '.$_SESSION[username].'. You passed all online test .We will contact soon and give you more detail</div>
          ';
          ...
          //send email with attach pdf
            ...
            //end
            //header
            $email_body='Herzlichen Glückwunsch '.$_SESSION[username].'.<br>Sie haben den kompletten Fernlehrgang bestanden.';
            ...
            mail($_SESSION['email'],"Herzlichen Gl&uuml;ckwunsch zum bestandenen Fernlehrgang",$message,$headers);
        }
     }
 return;
 }
 ...
 //Ajax for show test pass
  if(isset($_POST['id_user_test']))
  {
     ...
     $test=$row->test;
     $xml=simplexml_load_file("questions/question".$test.".xml") or die("Error: Ein XML Fehler ist vorhanden. Bitte informieren Sie den <a href='mailto:post@pixel-dusche.de?subject=Fehler in der XML&body=Bitte XML $test pruefen.'>Admin</a>.");
     $title=$xml->title;
     $color=$xml->color;
