...
#	elseif($islock) echo('SPÃ„TER NUR FÃœR BERECHTIGTE !!!!!!!!');
#	elseif($islock) echo('SPÃ„TER NUR FÃœR BERECHTIGTE !!!!!!!!');
...
/* vu nam */
  ...
  if(isset($_POST['register']))
 {
    ...
    if($row->uid!="")
    {
       echo "<p class='error'>User exist , please register another user</p>";
       $class='error';
    }
    ...
    if($row->uid!="")
    {
       echo "<p class='error'>email exist , please register another email</p>";
       $class='error';
    }
    if($class=="")
    {
        //insert table user
        $sql="insert into user(uname,pw,email,active,token) values('".$name."','".$password."','".$email."',0,'".$token."')";
        safe_query($sql);
        //send email to active
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $message="Please click link below to active your account<br><br>";
        $message.="<a href='".$morpheus["local"]."?token=".$token."'>".$morpheus["local"]."/?token=".$token."</a>";
        //echo $message;
        $subject="Active your account from ".$morpheus["local"]."";
        mail($email,$subject,$message,$headers);
        echo "Thanks for your registration ,a email has sent to your email to active account";
    }
 ...
 if(isset($_POST['test']))
 {
     ...
     if($_POST['status']==0)
     {
        ...
        //if enought 3 times
        if($row->count==3)
         echo 'You had faild 3 times for this test , You can not do this test anytime , Please try with another account';
        ...
        else
        {
            ...
            //if faild <2 times
              ...
              echo 'You had faild '.$count.' time for this test, You only have '.$rest_test.' time for this test';
     ...
     else
     {
        ...
        //echo $sql;
        safe_query($sql);
        echo 'Congratulation , you are pass this test , you will give next test in next time';
     }
 return;
 }
