...
function liste() {
	//// EDIT_SKRIPT
	$db = "morp_mitarbeiter m";
	$db2 = "morp_mitarbeiter_abt a";
	$id = "mid";
	...
	while ($row = mysqli_fetch_object($res)) {
		$edit = $row->$id;
		if($old != $row->bezeichnung) {
			$echo .= '<tr>
			<td valign="bottom"><p> &nbsp; &nbsp; &nbsp; &nbsp; * * * * <a href="?edit='.$edit.'">'.$row->bezeichnung	.'</a> * * * * </p></td>
		</tr>';
			$old = $row->bezeichnung;
		}
		$echo .= '<tr>
			<td width="450"><p><a href="?edit='.$edit.'">'.$row->$anz4	.' | '.$row->$anz	.' - '.$row->$anz3.'</a></p></td>
...
elseif ($delete) {
	$sql = "DELETE FROM morp_mitarbeiter WHERE mid=$delete";
	$res = safe_query($sql);
}
