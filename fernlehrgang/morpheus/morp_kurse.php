...
$arr_form = array(
	...
	array("colid", "Farbe", 'sel', 'morp_color', 'colname'),
#	array("tid", "Position", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
	array("beschreibung", "Beschreibung", '<textarea name="#n#" class="editor" style="width:800px; height:300px;">#v#</textarea>'),
#	array("fon", "Telefon", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
#	array("aid", "Abteilung", 'sel', 'morp_kurse_abt', 'bezeichnung'),
...
function liste() {
	...
	$sql = "SELECT * FROM $db k LEFT JOIN morp_color c ON k.colid=c.colid WHERE 1 ORDER BY ".$ord."";
	$res = safe_query($sql);

	while ($row = mysqli_fetch_object($res)) {
		$edit = $row->$id;
		$echo .= '<tr>
			<td width="450"><p><a href="?edit='.$edit.'">'.$row->$anz.' - '.$row->colname.'</a></p></td>
...
elseif ($delete) {
	$sql = "DELETE FROM morp_kurse WHERE kid=$delete";
	$res = safe_query($sql);
}
