...
$arr_form = array(
	array("wid", "Interne Seiten ID", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
	array("gruppe", "Warengruppe", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
	array("subline", "Name lang", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
	array("ansprechpartner", "Ansprechpartner f&uuml;r den Kunden", '<input type="Text" value="#v#" name="#n#" style="#s#">'),
 	array("beschreibung", "Beschreibung", '<textarea cols="80" rows="8" name="#n#">#v#</textarea>'),
 	array("xxx", "------", '<textarea cols="80" rows="8" name="#n#">#v#</textarea>'),
	array("img", "Foto (228 x 162)", 'foto', 'image', 'imgname', 6, 'gid'),
	array("img2", "Foto ", 'foto', 'image', 'imgname', 6, 'gid'),
...
function liste() {
	...
	while ($row = mysqli_fetch_object($res)) {	
		$edit = $row->$id;
		$echo .= '<tr>
			<td width="600"><p><a href="?edit='.$edit.'">'.$row->$anz.'</strong> / '.$row->$anz2.'</a></p></td>
...
';
	...
	foreach($arr_form as $arr) {	
		...
		elseif ($arr[2] == "foto") {
			...
			if ($row->$arr[0] && $arr[0] != "pdf") $echo .=  '<img src="../timthumb.php?zc2&w=200&src=images/produkt/'.$row->$arr[0].'"></a> &nbsp; &nbsp; <a href="?delimg='.$arr[0].'&edit='.$edit.'"><img src="images/delete.gif" width="9" height="10" alt="Bild löschen" border="0" hspace="6"></a>';
...
elseif ($delete) {
	$sql = "DELETE FROM morp_shop_wg WHERE sid=$delete";
	$res = safe_query($sql);	
}
