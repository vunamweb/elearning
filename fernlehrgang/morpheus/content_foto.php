...
function img_group ($inr, $edit, $back) {
...
if ($inr >= "0" && $imgid) {
	$query = "update nav set emotional=$imgid, edit=1 where navid=$edit";
	$result = safe_query($query);	
}
# bild rechts loeschen
elseif ($del) {
	# if ($imgid) $query = "update content set img" .$imgid ."=1, edit=1 where cid=$edit";
	$query = "update nav set emotional=0, edit=1 where navid=$edit";
	$result = safe_query($query);	
}
