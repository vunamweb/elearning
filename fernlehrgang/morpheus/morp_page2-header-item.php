...
if(isset($edit) && isset($id_pdf) || $neu)
 echo '<p><a href="morp_page2-header-item.php?id_pdf='.$id_pdf.'&language='.$language.'">&laquo; zur&uuml;ck</a></p>';
...
else if($update)
{
   ...
   //set field title or title_english will be inserted
    ...
    //
   $sql="update $db set $name_colum='".$name."',$value_colum='".$value."' ,id_page2_header=".$item." where id=".$id."";
   // echo $sql;
   $res = safe_query($sql);
...
else if($edit)
{
   edit($edit, $id_pdf);
    ...
    $db = "page2_header_item";
	$sql = "insert into $db(id_page2_header,$name_colum,$value_colum)values(".$item.",'".$name."' ,'".$value."') ";
    //echo $sql;
	$res = safe_query($sql);
    // show form
    $db = "page2_header";
...
function edit($edit, $id_pdf)
{
  ...
  $sql = "SELECT * FROM $db WHERE id_pdf=".$id_pdf." ORDER BY name";
    ...
    /* end */
        if($row->id_page2_header==$row1->id)
          $selected="selected";
        else
          $selected="";
...
function liste($id_pdf) {
	...
	$sql = "SELECT b.name,b.name_english,a.name AS gruppe,a.name_english AS gruppe_english,b.value,b.value_english,b.id FROM page2_header as a,page2_header_item as b  WHERE a.id_pdf=".$id_pdf."
    and a.id=b.id_page2_header order by a.name, b.name ";
	$res = safe_query($sql);
