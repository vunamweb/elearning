...
function letsgo($id_arr, $db, $arr, $rebuild) {
	...
	foreach($id_arr as $val) {
		...
		if ($db == "content" || $db == "coach") {
			if (eregi("tabelle@@", $row->text)) {
				$tab = explode("##", $row->text);
				foreach($tab as $brick) {
					if (eregi("tabelle@@", $brick)) {
						$brick = explode("@@", $brick);
						#echo "YES!$brick[1]<br>";
						$is_tabelle[] = $brick[1];
						#print_r($is_tabelle);
	...
	foreach($id_arr as $val) {
		if ($db != "delete") {
			$query = "SELECT * FROM $db where $arr[1]='$val'";
			$res   = safe_query($query);
			# echo "<br>anzahl: ".mysql_num_rows($res)."<br>";
			if (mysql_num_rows($res) > 0) $upd = "update";
			else $upd = "insert";
