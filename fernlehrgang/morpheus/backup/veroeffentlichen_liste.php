...
$db 	 = $_REQUEST["db"]; if (!$db) $db = "content";
...
elseif ($db != "delete") $query = "SELECT * FROM $db c, z_protokoll z, user u WHERE 
									c.$arr[1]=z.id AND 
									u.uid=z.uid AND
									z.db='$db' AND 
									c.edit=1
...
if (mysql_num_rows($res) > 0) echo $live;
...
while ($row = mysql_fetch_object($res))	{
	...
	if (!in_array($id, $tmp_arr)) { 
		$tmp_arr[] = $id;
		if ($db == "content" || $db == "nav") {
			$que = "SELECT * FROM nav where navid=$id";
			$re  = safe_query($que);
			...
			if ($par) {
				for ($n=0; $n<4;$n++) {
					if ($par) {
						$que = "SELECT * FROM nav where navid=$par";
						$re  = safe_query($que);
						$ro = mysql_fetch_object($re);
						$par = $ro->parent;
		...
		if ($db == "content") $anzeige = $anzeige.'</a></td><td width=80><!-- <a href="../frameset.php?'.$vors.'" target="_blank">vorschau</a> --></td><td><a href="veroeffentlichen.php?rebuild='.$id.'">wiederherstellen</a>';
