...
if(isset($edit) && isset($id_pdf) || $neu)
 echo '<p><a href="morp_page2-header-item.php?id_pdf='.$id_pdf.'">&laquo; zur&uuml;ck</a></p>';
...
{
   $db = "page2_header_item";
   $sql="update $db set name='".$name."',value='".$value."' ,id_page2_header=".$item." where id=".$id."";
   // echo $sql;
   $res = safe_query($sql);
...
else if($edit)
{
   edit($edit, $id_pdf);
    ...
    $db = "page2_header_item";
	$sql = "insert into $db(id_page2_header,name,value)values(".$item.",'".$name."' ,'".$value."') ";
    //echo $sql;
	$res = safe_query($sql);
    // show form
    $db = "page2_header";
...
function liste($id_pdf) {
	...
	$sql = "SELECT b.name,a.name AS gruppe,b.value,b.id FROM page2_header as a,page2_header_item as b  WHERE a.id_pdf=".$id_pdf."
