...
$res = safe_query($sql);
$row = mysqli_fetch_object($res);
$page1_text_top=$row->value_english;
if($page1_text_top=="")
 $page1_text_top="No tranlate\nNo tranlate";
...
$res = safe_query($sql);
$row = mysqli_fetch_object($res);
$page2_text=$row->value_english;
if($page2_text=="")
 $page2_text="No tranlate"; 
...
while ($row = mysqli_fetch_object($res)) {
  if($row->title_english=="")
    $row->title_english="No tranlate";
  if($row->text_english=="")
    $row->text_english="No tranlate";
...
while ($row = mysqli_fetch_object($res)) {
  if($row->name_english=="")
   $row->name_english="No tranlate";
  $html.='<div class="detail">
  <p style="font-size:18px;color:#706F6F;margin-top:25px;margin-bottom:7px;font-family:Univers-57-Condensed">'.$row->name_english.'</p>
  ...
  while ($row1 = mysqli_fetch_object($res1)) {
     if($row1->name_english=="")
       $row1->name_english="No tranlate";
     if($row1->value_english=="")
       $row1->value_english="No tranlate";
     if($k%2==1)
      $style='style="background:#'.$color2.';height:45px;font-size:12px"';
     else
      $style='style="height:45px;font-size:12px"';
     $html.='<tr '.$style.'>
                <td style="font-weight:600;border-right:1.5px solid #fff" valign="top" width="50%">'.$row1->name_english.'</td>
                <td valign="top" width="50%">'.$row1->value_english.'</td>
              </tr>';
     $k++;
  }
  $html.='</table></div>';
...
while ($row = mysqli_fetch_object($res)) {
  if($row->name_english=="")
   $row->name_english="No tranlate";
  $html.='<div class="detail">
   <p style="font-size:18px;color:#706F6F;margin-bottom:7px;margin-top:25px;font-family:Univers-57-Condensed">'.$row->name_english.'</p>
  ...
  while ($row1 = mysqli_fetch_object($res1)) {
    ...
    if($row1->name_english!="")
    {
         ...
         if($k%2==1)
            ...
            $style='style="height:45px;font-size:12px"';
         $html.='<tr '.$style.' >
                    <td style="font-weight:600;border-right:1.5px solid #fff" width="30%">'.$row1->name_english.'</td>
                    <td  width="30%">'.$row1->value_english.'</td>
                  </tr>';
         $k++;
    }
  }
...
<!-- page3  !-->
       ...
       <div class="page3">
          ...
          <div style="clear:both"></div>
          <div class="detail">
             <p style="font-size:18px;color:#706F6F;margin-bottom:7px;margin-top:40px;font-family:Univers-57-Condensed">ALLERGY LIST</p>
             <table width="100%">
              <tr style="background:#'.$color.';color:#fff;font-size:14px;font-weight:700">
                <td width="50%">Allergen</td>
                <td width="25%">with recipe</td>
                <td width="25%">without recipe</td>
              </tr>';
//page3
$db = "checkbox_english";
$sql = "SELECT * FROM $db";
...
<div class="footer-top">
  <div style="float:left;width:40%">
    <div style="">
        <p style="color:#706F6F;font-size:22px;margin-bottom:8px;font-family:Univers-57-Condensed">Suitable for specific diets:</p>
        <p style="line-height:1em">Coeliac<br>Vegetarian<br>Vegan</p>
    </div>
    <div style="margin-top:25px">
        <p style="color:#706F6F;font-size:22px;margin-bottom:8px;font-family:Univers-57-Condensed">Retraceability:</p>
        <p style="line-height:1em">
          The traceability of the product is guaranteed through the lot numbers of each sales contract
        </p>
    </div>
  </div>
  <div style="float:left;padding-left:20px;width:50%">
    <div style="margin_bottom:20px">
          <p style="color:#706F6F;font-size:22px;margin-bottom:10px;font-family:Univers-57-Condensed">Legal information and Certificates:</p>
          <p style="line-height:1em;margin-bottom:10px;fontweight:300">The product conforms to the relevant recent German and EC-law. ISO9001:2008;</p>
          <p style="line-height:1em;">ISO22000:2005 Halal; Kosher, NON-GMO of SGS; IP and HACCP</p>
    </div>
    <p style="line-height:1em;font-family:Univers-55-Light">The specification valid until revoked and replace all previous issues.<p>
  </div>
</div>
<!-- footer bottom  !-->
<div style="clear:both"></div>
